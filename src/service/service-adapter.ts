...
export class SubagentsServiceAdapter implements SubagentsService {
  ...
  spawn(type: string, prompt: string, options?: SpawnOptions): string {
    ...
    if (backend === "cursor" && options?.model) {
      throw new Error("SpawnOptions.model is Pi-only when backend=cursor; use cursorModel.");
    }
    if (backend === "pi" && options?.cursorModel) {
      throw new Error("SpawnOptions.cursorModel requires backend=cursor.");
    }
    const model = backend === "pi" ? this.resolveModelOption(options?.model) : undefined;
    ...
    return this.manager.spawn(snapshot, type, prompt, {
      ...
      permissionMode: backend === "cursor" ? options?.permissionMode ?? "deny" : undefined,
  ...
  async steer(id: string, message: string): Promise<boolean> {
    ...
    return outcome.kind === "delivered" || outcome.kind === "buffered";
  }
